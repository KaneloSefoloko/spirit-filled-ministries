...
export default function BibleQuiz() {
    ...
    const [checkingParticipant, setCheckingParticipant] =
        useState(false);
    ...
    const [timedOutQuestions, setTimedOutQuestions] =
        useState({});

    // Keeps the active timer tied to the question it belongs to.
    const timerQuestionRef = useRef(null);

    // Prevents timeout and manual navigation from fighting.
    ...
    useEffect(() => {
        async function loadQuiz() {
            try {
                ...
                console.error(
                    "❌ Bible quiz loading error:",
                    err
                );

                setError(
                    err?.message ||
                    "Unable to load the Bible quiz."
                );
            } finally {
                setLoading(false);
            }
...
// QUIZ DEADLINE
// =========================================================
// end_at is a timestamptz from Supabase.
// JavaScript treats it as an absolute moment in time,
// so the cutoff works correctly regardless of the
// participant's timezone.
    ...
    useEffect(() => {
        ...
        // A timed-out question remains locked.
        if (timedOutQuestions[question.id]) {
            return;
        }

        // Tie this timer to this question.
        ...
        const timer =
            window.setInterval(() => {
                // This timer is no longer allowed to act
                // if the participant has moved elsewhere.
                if (
                    timerQuestionRef.current !==
                    question.id
                ) {
                ...
                if (
                    ...
                    // Permanently lock this question.
                    ...
                    if (
                        ...
                        // New question starts with a fresh timer.
                        setTimeLeft(
                            QUESTION_TIME_LIMIT
                        );
                        ...
                        //
                        // DO NOT AUTO-SUBMIT.
                        // Leave question locked.
                        // Participant manually submits.
                        // =================================================
    ...
    function selectAnswer(
        ...
        // A timed-out question can never be answered.
        if (
            submitting ||
        ...
        // Timer must also still be active.
        if (timeLeft <= 0) {
            return;
        }
    ...
    function goToNextQuestion() {
        ...
        // You can only move forward if:
        // 1. An answer has been selected, OR
        // 2. The question timed out.
        if (
            !selectedAnswers[
                currentQuestionData?.id
                ] &&
    ...
    async function startQuiz(e) {
        ...
        if (
            ...
            alert(
                "Please select your branch or choose Visitor."
            );
            return;
        }

        try {
            setCheckingParticipant(
                true
            );
            ...
            console.error(
                "❌ Duplicate quiz check error:",
                err
            );

            setError(
                err?.message ||
                "Unable to verify your quiz status."
            );
        } finally {
            setCheckingParticipant(
                false
    ...
    //
    // Timed-out questions are submitted with null.
    // =========================================================
    ...
    async function submitQuiz() {
        ...
        try {
            ...
            // PREPARE ANSWERS
            //
            // Timed-out questions receive null.
            // -------------------------------------------------
...
// =========================================================
    ...
    if (quizExpired && !quizResult) {
        return (
            <div className="min-h-screen bg-[#f7f7f9] flex items-center justify-center px-5">
                <div className="w-full max-w-lg bg-white rounded-[30px] shadow-xl border border-gray-100 p-8 sm:p-12 text-center">

                    <div className="w-20 h-20 mx-auto mb-7 rounded-[24px] bg-purple-50 flex items-center justify-center">
                    ...
                    <p className="text-gray-500 leading-relaxed">
                        This week's Bible Quiz has now closed.
                    ...
                    <button
                        ...
                        className="mt-7 inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl bg-purple-600 text-white text-sm font-semibold shadow-lg shadow-purple-600/20 hover:bg-purple-700 transition-all"
                    >
                        View Leaderboard
                            ...
                            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                                ...
                                <div className="flex flex-col sm:flex-row gap-3">
                                    <button
                                        ...
                                        className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl bg-purple-600 text-white text-sm font-semibold shadow-lg shadow-purple-600/20 hover:bg-purple-700 transition-all"
                                    >
                                        View Leaderboard
                                        <ChevronRight className="w-4 h-4" />
                                    </button>
                                    ...
                                    <button
                                        ...
                                        className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl border border-gray-200 bg-white text-gray-700 text-sm font-semibold hover:bg-gray-50 transition-all"
                                    >
                                        Return Home
                                    </button>
                                </div>
                    ...
                    <div className="bg-white rounded-[30px] border border-gray-100 shadow-[0_15px_50px_rgba(15,23,42,0.07)] overflow-hidden">
                        ...
                        <div className="p-7 sm:p-10 text-center">
                            ...
                            <button
                                ...
                                className="mt-7 w-full inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl bg-purple-600 text-white text-sm font-semibold hover:bg-purple-700 transition-all"
                            >
                                View Leaderboard
                                <ChevronRight className="w-4 h-4" />
                            </button>
                    ...
                    <div className="bg-white rounded-[30px] shadow-[0_15px_50px_rgba(15,23,42,0.07)] border border-gray-100 p-6 sm:p-10">
                        ...
                        >
                            ...
                            <button
                                ...
                                className="w-full inline-flex items-center justify-center gap-3 px-6 py-4 rounded-2xl bg-purple-600 text-white font-semibold shadow-lg shadow-purple-600/20 hover:bg-purple-700 transition-all disabled:bg-purple-300 disabled:cursor-not-allowed"
                            >
                                {checkingParticipant ? (
                                    <>
            ...
            <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 lg:py-16">
                ...
                <div className="mb-6 sm:mb-8">
                    <div className="flex items-center justify-between gap-4 mb-4">
                        ...
                        <div
                            className={`relative shrink-0 w-[76px] h-[76px] sm:w-[88px] sm:h-[88px] rounded-full transition-all duration-300 ${
                                questionTimedOut || timerIsUrgent
                                    ? "animate-pulse"
                                    : ""
                            }`}
                            style={{
                                background: `conic-gradient(
                                    ...
                                    rgba(255,255,255,0.14) ${timerPercentage}% 100%
                                )`,
                            }}
                            aria-label={
                                questionTimedOut
                        ...
                        >
                            <div className="absolute inset-[5px] rounded-full bg-indigo-950 border border-white/10 flex flex-col items-center justify-center shadow-inner">
                                <Clock3
                                ...
                                <span className="text-base sm:text-xl font-black leading-none tabular-nums text-white">
                                    {questionTimedOut ? "—" : timeLeft}
                                </span>

                                <span className="text-[7px] sm:text-[8px] uppercase tracking-[0.18em] font-bold text-white/45 mt-1">
                                    {questionTimedOut ? "Time Up" : "Seconds"}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                {showTimeUp && (
                    <div className="mb-5 rounded-2xl bg-amber-50 border border-amber-200 px-5 py-4 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-amber-700 mt-0.5">
                                ...
                                1
                                    ...
                                    : "This question is locked. You can now submit your quiz."}
                            </p>
                        </div>
                    </div>
                )}
                ...
                <div className="bg-white rounded-[26px] sm:rounded-[32px] border border-gray-100 shadow-[0_15px_50px_rgba(15,23,42,0.07)] overflow-hidden">
                    ...
                    <div className="px-5 sm:px-8 md:px-10 lg:px-12 py-7 sm:py-9">
                        ...
                        <div className="grid gap-3 sm:gap-4">
                            {question.bible_quiz_answers.map(
                                (answer) => {
                                    const isSelected =
                                        selectedAnswer ===
                                        answer.id;
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`group relative w-full text-left rounded-2xl border-2 p-4 sm:p-5 transition-all duration-200 focus:outline-none focus-visible:ring-4 focus-visible:ring-purple-500/20 disabled:cursor-not-allowed ${
                                                isSelected
                                        ...
                                        >
                                            <div className="flex items-center gap-3.5 sm:gap-4">
                                                <div
                                                    className={`w-11 h-11 sm:w-12 sm:h-12 shrink-0 rounded-xl flex items-center justify-center font-bold text-sm transition-all duration-200 ${
                                                        isSelected
                    ...
                    <div className="border-t border-gray-100 bg-gray-50/80 px-5 sm:px-8 md:px-10 lg:px-12 py-5 sm:py-6">
                        <div className="flex items-center justify-between gap-3">
                            <button
                                ...
                                className="inline-flex items-center justify-center gap-2 px-4 sm:px-6 py-3 sm:py-3.5 rounded-xl border border-gray-200 bg-white text-sm font-semibold text-gray-700 hover:bg-gray-100 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                            >
                            ...
                            1 ? (
                                <button
                                    ...
                                    className="inline-flex items-center justify-center gap-2 px-5 sm:px-7 py-3 sm:py-3.5 rounded-xl bg-gray-950 text-white text-sm font-semibold shadow-lg shadow-gray-900/10 hover:bg-purple-700 transition-all disabled:bg-gray-300 disabled:shadow-none disabled:cursor-not-allowed"
                                >
                                    <span>
                                        Next
                                    </span>
                                ...
                                <button
                                    ...
                                    className="inline-flex items-center justify-center gap-2 px-5 sm:px-7 py-3 sm:py-3.5 rounded-xl bg-emerald-600 text-white text-sm font-semibold shadow-lg shadow-emerald-600/10 hover:bg-emerald-500 transition-all disabled:bg-gray-300 disabled:shadow-none disabled:cursor-not-allowed"
                                >
                                    Submit Quiz
                    ...
                    <div className="relative w-full max-w-md bg-white rounded-[28px] shadow-2xl border border-gray-100 overflow-hidden">
                        <div className="p-7 sm:p-9">
                            ...
                            <p className="mt-4 text-sm sm:text-base text-gray-500 leading-relaxed">
                                {unansweredCount ===
                                0
                                    ? `You have answered all ${questions.length} questions. Once submitted, your answers cannot be changed.`
                                    : `You have answered ${answeredCount} of ${questions.length} questions. ${unansweredCount} ${
                                        unansweredCount ===
                                        1
                                            ? "question will"
                                            : "questions will"
                                    } be submitted unanswered. Once submitted, your answers cannot be changed.`}
                            </p>
                            ...
                            <div className="flex flex-col-reverse sm:flex-row gap-3 mt-7">
                                <button
                                    ...
                                    className="flex-1 px-5 py-3.5 rounded-xl border border-gray-200 bg-white text-sm font-semibold text-gray-700 hover:bg-gray-50 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    Go Back
                                </button>
                                ...
                                <button
                                    ...
                                    className="flex-1 inline-flex items-center justify-center gap-2 px-5 py-3.5 rounded-xl bg-emerald-600 text-white text-sm font-semibold shadow-lg shadow-emerald-600/10 hover:bg-emerald-500 transition-all disabled:bg-gray-300 disabled:shadow-none disabled:cursor-not-allowed"
                                >
                                    {submitting ? (
                                        <>
